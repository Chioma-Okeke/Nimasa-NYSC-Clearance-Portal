...
function ReviewedForms({ employee }: { employee: IEmployeeCreationResponse }) {
    ...
    const { data: reviewedForms = [], isLoading } = useQuery(getClearanceFormsByStatusQueryOpt(employee?.role || "", FORM_STATUSES.PENDING_HOD))
                        ...
                        {reviewedForms.map((form) => (
                            ...
                            >
                                <div className="space-y-2">
                                    ...
                                    {form.supervisorDate && (
                                        <p className="text-sm text-muted-foreground">
                                            Reviewed: {formatDate(form.supervisorDate)}
                                        </p>
                                    )}
